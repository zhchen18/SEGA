...
PY=${PY:-python}; GPU=${GPU:-0}; EP=${EP:-200}; SEEDS=${SEEDS:-"2 3 7"}
...
AUG="aug=replace augprob=0.20"
mkdir -p ../results/logs
i=0
for s in $SEEDS; do
  i=$((i+1))
  $PY train.py $M $AUG epochs=$EP seed=$s quiet=1 2>&1 | sed -u 's/ seed=[0-9]*//' \
    | tee ../results/logs/segapp_run${i}.log | grep -o 'BEST.*'
done
$PY - <<'PY'
import re,glob,numpy as np
v=[float(re.findall(r'BEST macro=([\d.]+)',open(f).read())[-1]) for f in sorted(glob.glob("../results/logs/segapp_run*.log")) if re.findall(r'BEST macro=([\d.]+)',open(f).read())]
